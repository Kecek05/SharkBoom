

public static class CloudCodeRefs
{

    //Generic
    public const string ARGUMENT_PLAYERID = "playerId";
    public const string ARGUMENT_PROJECT_ID = "gameProjectId";
    public const string PROJECT_ID = "01563be5-25e2-47ed-b519-012967e3d8e3";

    //Is In Match
    public const string GET_ISINMATCH_ENDPOINT = "GetIsInMatch";
    public const string SET_ISINMATCH_ENDPOINT = "SetIsInMatch";
    public const string ARGUMENT_ISINMATCH = "isInMatch";

    //Player Match Connection
    public const string SET_PLAYER_MATCH_CONNECTION_ENDPOINT = "SetPlayerMatchConnection";
    public const string ARGUMENT_IP = "ipServer";
    public const string ARGUMENT_PORT = "portServer";

    //Get Match Connection
    public const string GET_PLAYER_PORT_SERVER_ENDPOINT = "GetPlayerPortServer";
    public const string GET_PLAYER_IP_SERVER_ENDPOINT = "GetPlayerIpServer";

    //Pearls
    public const string ADD_PEARLS_ENDPOINT = "AddSavePlayerPearls";
    public const string GET_PEARLS_ENDPOINT = "GetPlayerPearls";
    public const string ADD_PEARLS_ARGUMENT_PEARLS = "pearls";

}
